...
for ($jj=0; $jj<$ntrans; $jj++){  # loop over transitions
	...
	print PROGRESSLOG scalar(localtime)."\t sub: constructconf \n";
	&constructconf;
	...
	print PROGRESSLOG scalar(localtime)."\t sub: rcsfexcitationprepare \n";
	&rcsfexcitationprepare;

	# construct input file for rcsfexcitation
	print PROGRESSLOG scalar(localtime)."\t sub: rcsfexcitationinputconstruct\n";
	...
	if($cudflag == 1){
	    ...
	    # construct input file for rcsfexcitation
	    print PROGRESSLOG scalar(localtime)."\t sub: rcsfexcitationinputconstruct2\n";
	    &rcsfexcitationinputconstruct2;

	    # run rcsfexciation and rcsfgenerate
	    print PROGRESSLOG scalar(localtime)."\t SYSTEM: rcsfexcitation < input2.log \n";
	    ...
	    print PROGRESSLOG scalar(localtime)."\t SYSTEM: rcsf.out rcsl.inp \n";
	    system("cp rcsf.out rcsl.inp");

	    print PROGRESSLOG scalar(localtime)."\t sub: jjcudinputconstruct\n";
	    &jjcudinputconstruct;
	    ...
	    print PROGRESSLOG scalar(localtime)."\t SYSTEM: jjcud < input_jjcud \n"; 
	    system("jjcud < input_jjcud");
	    print PROGRESSLOG scalar(localtime)."\t SYSTEM: cp test_cud.c rcsf.inp \n"; 
	    system("cp test_cud.c rcsf.inp");
	    

	    print PROGRESSLOG scalar(localtime)."\t sub: runrcsfblock \n"; 
	    &runrcsfblock;
	    
	    print PROGRESSLOG scalar(localtime)."\t sub: readblockinfo2 \n"; 
	    &readblockinfo2;
	...
	if($emptyshellflag == 1 && $recalcstate[$ii] == 1){
	    print PROGRESSLOG scalar(localtime)."\t sub: constructrcsfinp \n"; 
	    &constructrcsfinp;
	}
	...
	if($recalcstate[$ii] == 1){
		...
		if($defaultiniflag == 1){
		    ...
		    print PROGRESSLOG scalar(localtime)."\t sub: rcsfnondefault \n"; 
		    &rscfnondefault;
		}
	    }else{
		if($defaultfinflag == 1){
	...
	if($emptyshellflag == 1 && $recalcstate[$ii] == 1){
	    print PROGRESSLOG scalar(localtime)."\t SYSTEM: cp rcsf.inp.nocan rcsf.inp \n"; 
	    system("cp rcsf.inp.noncan rcsf.inp");
	    print PROGRESSLOG scalar(localtime)."\t SYSTEM: rangular < input_rangular \n"; 
	    system("rangular < input_rangular");
	    print PROGRESSLOG scalar(localtime)."\t sub: rwfnestimateinputconstruct2\n"; 
	...
	if($recalcstate[$ii] == 1){
	    print PROGRESSLOG scalar(localtime)."\t sub: jj2lsjinputconstruct \n"; 
	...
	#run jjgen_ratip
	if($recalcstate[$ii] == 1){
	    print PROGRESSLOG scalar(localtime)."\t SYSTEM: jjgen_ratip < input_jjgen_ratip \n"; 
	    system("jjgen_ratip < input_jjgen_ratip");
	}
	
	# construct input file for cvtmixratip
	if($recalcstate[$ii] == 1){
	    print PROGRESSLOG scalar(localtime)."\t sub: cvtmixratipinputconstruct \n"; 
	...
	print PROGRESSLOG scalar(localtime)."\t sub: input_rbiotransform($initialstate,$finalstate) \n"; 
	input_rbiotransform($initialstate,$finalstate);
	print PROGRESSLOG scalar(localtime)."\t SYSTEM: rbiotransform < rbiotransform.inp \n"; 
	system("rbiotransform < rbiotransform.inp");

	print PROGRESSLOG scalar(localtime)."\t sub: input_rtransition($initialstate,$finalstate) \n"; 
	input_rtransition($initialstate,$finalstate);
	#system("rtransition < rtransition.inp");
	print PROGRESSLOG scalar(localtime)."\t SYSTEM: rtransition2 < rtransition.inp \n"; 
	system("rtransition2 < rtransition.inp");
	
	print PROGRESSLOG scalar(localtime)."\t sub: designator \n"; 
	&designator;
	print PROGRESSLOG scalar(localtime)."\t sub: readxreossum_rdr($initialstate,$finalstate) \n"; 
...
sub hashtags
{
    ...
    # RDR 191121
    %testhash7 = ( '1' => 'K',    '3' => 'L1',  '5' => 'L2',  '6' => 'L3',  '8' => 'M1',  
		   '10' => 'M2',  '11' => 'M3', '13' => 'M4', '14' => 'M5', '16' => 'N1', 
		   '18' => 'N2',  '19' => 'N3', '21' => 'N4', '22' => 'N5', '24' => 'N6', 
		   '25' => 'N7',  '27' => 'O1', '29' => 'O2', '30' => 'O3', '32' => 'O4', 
		   '33' => 'O5',  '35' => 'O6', '36' => 'O7', '38' => 'O8', '39' => 'O9', 
		   '41' => 'P1',  '43' => 'P2', '44' => 'P3', '46' => 'P4', '47' => 'P5',
		   '49' => 'P6',  '50' => 'P7', '52' => 'P8', '53' => 'P9', '55' => 'P10', 
		   '56' => 'P11', '58' => 'Q1', '60' => 'Q2', '61' => 'Q3',    
	);
}
...
sub constructconf
{
	...
	@ssocc[$i] = @orbsplit[1];                      # get occupancy for given orb (from original config)
	$sumocc = $sumocc + @ssocc[$i];                 # tot occ all orb
	$length = length(@orbsplit[0]);
	if($length == 2){
	    @ssn[$i] = substr(@orbsplit[0], 0, 1);      # get n,l and nl data
	...
	if($ii == 0){  # if initial state
		...
		@ssocc[$i]--;                           # reduce occ num for orb initial
		$sumocc--;
	    }
	}else{  # if final state
...
sub jjgen_ratipinputconstruct
{
	...
	print FILE "$initialstate\n";
	if($emptyshellflag == 1){
	    print PROGRESSLOG scalar(localtime)."\t\t subSYS: cp rscf.inp.ratip $initialstatec \n"; 
	    system("cp rcsf.inp.ratip $initialstatec");
	}
    }else{
	print FILE "$finalstate\n";
	if($emptyshellflag == 1){
	    print PROGRESSLOG scalar(localtime)."\t\t subSYS: cp rscf.inp.ratip $finalstatec \n"; 
...
sub readxreossum_rdr{
	...
	if (($line =~m/Babushkin/ || $line =~m/Magnetic/)) {
	    ...
	    #      if($channel40[$transno]==6){
	    #	  print "$transno $channel[0] $k $docc1 ------------------------------------ \n";
	    #      }
